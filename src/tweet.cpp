...
#include "objects/objects_template.hpp"
...
void Tweet_data::construct(const boost::property_tree::ptree& tree) {
    auto contributors_tree = tree.get_child("contributors", boost::property_tree::ptree());
    for(const auto& pair : contributors_tree) {
        if(!pair.second.empty()) {
        contributors.push_back(std::make_unique<User>(pair.second));
        }
    }
    coordinates = Coordinates{tree.get_child("coordinates", boost::property_tree::ptree())};
    created_at = tree.get<std::string>("created_at", "");
    current_user_retweet_id = tree.get<std::int64_t>("current_user_retweet.id", -1);
    current_user_retweet_id_str = tree.get<std::string>("current_user_retweet.id_str", "");
    ...
    in_reply_to_screen_name = tree.get<std::string>("in_reply_to_screen_name", "");
    in_reply_to_status_id = tree.get<std::int64_t>("in_reply_to_status_id", -1);
    in_reply_to_status_id_str = tree.get<std::string>("in_reply_to_status_id_str", "");
    in_reply_to_user_id = tree.get<std::int64_t>("in_reply_to_user_id", -1);
    in_reply_to_user_id_str = tree.get<std::string>("in_reply_to_user_id_str", "");
    ...
    auto quoted_status_tree = tree.get_child("quoted_status", boost::property_tree::ptree());
    ...
    for (const auto& pair : scopes_tree) {
        scopes.push_back(std::make_pair(pair.first, pair.second.get_value<std::string>("")));
    }
    retweet_count = tree.get<int>("retweet_count", -1);
    retweeted = tree.get<bool>("retweeted", false);
    auto retweeted_status_tree = tree.get_child("retweeted_status", boost::property_tree::ptree());
    ...
    auto wic_tree = tree.get_child("withheld_in_countries", boost::property_tree::ptree());
